...
backnd gamer search --json --created-from 2026-06-01
```
...
```
backnd blocking block --dry-run --json --id <gamer-id>
```
...
사용자 확인 후 실행:
```
backnd blocking block --json --id <gamer-id> --reason "..."
```

## 주의사항

- `blocking`은 게임 전체 접근 차단. 채팅만 제한하려면 `chat ban`을 사용 (content.md 참조).
- 차단 사유(`--reason`)를 항상 기록해 이후 검토에 활용.
- 실수로 차단한 경우: `backnd blocking unblock --id <gamer-id>`로 해제 가능.
